...
public class LocalRowStore
{
  ...
  void writePath(Map data, String name)
  {
    ...
    try{
      m.put("parade.path", new File(state.getProperty(name).trim()).getCanonicalPath());
	...
	if(key.startsWith(propName))
	  m.put("parade."+key.substring(propName.length()), state.get(key));
      }
  ...
  synchronized void readState(javax.servlet.jsp.PageContext pc)
  {
	...
	{
	  String contextPath=((javax.servlet.http.HttpServletRequest)pc.getRequest()).getContextPath();
  ...
  public  synchronized void addParadeRow(java.util.Map data, javax.servlet.jsp.PageContext pc) throws ParadeException
  {
    String s[]=pc.getRequest().getParameterValues("parade.row");
    if(s==null ||s.length!=1 || s[0].trim().indexOf(" ")!=-1)
      throw new ParadeException("missing name, multiple names, or name containing spaces");
    ...
    try{
      state.save(new FileOutputStream(stateFile), "rows");
    }catch(IOException e){ throw new ParadeException("could not save state: "+e); }
  }
}
