...
public class CvsManager {
    ...
    public void readUserAndModule(Map row) {
        ...
        try {
            s = new BufferedReader(new FileReader(path + File.separator + "CVS"
                    + File.separator + "Repository")).readLine();
        } catch (IOException e) {
    ...
    public void readCVSEntries(java.util.Map paradeRow, java.util.Map data,
            javax.servlet.jsp.PageContext pc) throws ParadeException {
        File f = new File((paradeRow.get("parade.path") + "/"
                + paradeRow.get("file.path") + "CVS/Entries").replace('/',
                File.separatorChar));
        if (!f.exists()) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if (line.startsWith("/")) {
                    ...
                    if (date.equals("dummy timestamp")) {
                        row.put("cvs.status",
                                revision.startsWith("-") ? DELETED : ADDED);
                        continue;
                    }
                    ...
                    row.put("cvs.status", l > 0 ? LOCALLY_MODIFIED
                            : NEEDS_UPDATE);
                    // stupid windows bug
                    // if(l<0)
            ...
            throw new ParadeException(e.getMessage());
        }
    }

    public void readCVSIgnore(java.util.Map paradeRow, java.util.Map data,
            javax.servlet.jsp.PageContext pc) throws ParadeException {
        if (paradeRow.get("cvs.dir").equals("no"))
            return;
        File f = new File((paradeRow.get("parade.path") + "/"
                + paradeRow.get("file.path") + ".cvsignore").replace('/',
                File.separatorChar));
        if (!f.exists())
            return;
    ...
    public void readCVSCheckUpdate(java.util.Map paradeRow, java.util.Map data,
            javax.servlet.jsp.PageContext pc) throws ParadeException {
        Vector v = new Vector();
        v.addElement("cvs");
        v.addElement("-l");
        ...
        // if(Config.exec("cvs -n -l update -l -d -P",
        // (File)pc.findAttribute("file.baseFile"),
        ...
        while (st.hasMoreTokens()) {
            ...
            if (s.startsWith("M ")) {
                ...
                // hmmm. all other files that seem locally modified but are not
                // should be marked OK
                continue;
            }
            if (s.startsWith("U ")) {
