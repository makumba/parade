...
public class SourcePosition extends Observable
{
  ...
  public SourcePosition(int start){ this.start=start; this.end=-2; }

  /** Initialize a region */
  public SourcePosition(int start, int end){ this.start=start; this.end=end; }

  /** Initialize a position or a region in a text */
  public SourcePosition(String text, int sLine, int sColumn, int eLine, int eColumn)
  { 
    if(sLine<=0)
      {
	start=-2;
    ...
    /** if we have an end but no column, we extend the region to the end of the line */
  ...
  public boolean isSet(){return start>=0; }

  /** Tell if this position is a region (both start and end are set) */
  public boolean isRegion() { return isSet() && end>=0 ; }
  
  /** Get the line of this position in the given text */
  public int getLine(String text){ return getLineAndColumn(text)[0]; }

  /** Get the column of this position in the given text */
  public int getColumn(String text){ return getLineAndColumn(text)[1]; }
  ...
  public int getRegionEnd(){ return end; }
}
