...
public class FileManager {
    ...
    public void setParadeSimple(java.util.Map row,
        ...
        // nothing for now
        // we could e.g. compute the newest file from path/ on (attention,
        // ignore tomcat logs and makumba.jar...
    }

    public synchronized void findPathSimple(java.util.Map row,
    ...
    public File initFileOperation(java.util.Map data,
            javax.servlet.jsp.PageContext pc) throws ParadeException {
        List ignored = new ArrayList();
        pc.setAttribute("file.readingTime", new Long(new java.util.Date()
                .getTime()), javax.servlet.jsp.PageContext.REQUEST_SCOPE);
        pc.setAttribute("file.ignoredList", ignored,
                javax.servlet.jsp.PageContext.REQUEST_SCOPE);
        Map row = (Map) pc.findAttribute("parade.rowData");
        File f = new File(row.get("parade.path") + "/"
                + (String) row.get("file.path"));
        return f;
    }
    ...
    public void readFiles(java.util.Map data, javax.servlet.jsp.PageContext pc)
        ...
        if (!f.isDirectory())
            return;
        pc.setAttribute("file.baseFile", f,
                javax.servlet.jsp.PageContext.REQUEST_SCOPE);
    ...
    public void findAllSubdirs(String path, File f, java.util.Map data,
            javax.servlet.jsp.PageContext pc) {
        ArrayList ignored = (ArrayList) pc.findAttribute("file.ignoredList");
        File[] dir = f.listFiles();
        for (int i = 0; i < dir.length; i++) {
    ...
    public void setFileDataSimple(java.util.Map row,
            javax.servlet.jsp.PageContext pc) {
