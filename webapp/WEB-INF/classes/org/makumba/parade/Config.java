...
import org.makumba.view.jsptaglib.HtmlUtils;
...
public class Config
{
  ...
  /** iterate thru the list of manager objects given by a configuration property 
   * e.g. parade.managers= org.makumba.parade.xxx..., 
   */
  ...
  /** read the data of a domain
   * the read<domainname> operation is invoked
   */
  public static Map readDomainData(String domain, PageContext pc) throws ParadeException
  {
    Map data= new HashMap();
    invokeOperation(data, domain, "read"+Character.toUpperCase(domain.charAt(0))+
		    domain.substring(1), pc);
    return data;
  }
  
  /** invoke an operation on a parade domain
   * <domainname>.managers should be defined 
   * first, readDomainData is called
   * then the operation as such is invoked on the obtained data 
   */
  public static Map invokeOperation(String domain, String operationName, PageContext pc) throws ParadeException
  {
    Map data;
    invokeOperation(data=readDomainData(domain, pc), domain, operationName, pc);
    return data;
  }

  static Map empty=new HashMap(1);

  /** respond to a form. this will result in invoking an operation
   * taken from the "op" form parameter
   * <domainname>.managers should be defined 
   * first, the data is obtained, and filtered with the "entry" http parameter
   * then the operation indicated by "op" is called
   */
  public static String startPage(String domain, PageContext pc) throws ParadeException, javax.servlet.ServletException, IOException
  {
    String responseText="";
    ...
    /* 
       login comes later
       String s= Main.checkLogin(pageContext);
       if(s!=null) { // will do some http login here and we ignore rest of the page
       ((HttpServletResponse)pageContext.getResponse()).sendError(401, s);
  ...
  /** the core invokeOperation method, called by all other methods
   * for all managers the following methods are looked for and called:
   * <operationName>(Map allData, PageContext pc) 
   * <operationName>Simple(Map oneRow, PageContext pc) 
   */
  static public void invokeOperation(Map data, String domain, String operationName, PageContext pc) throws ParadeException
  {
    Object[] args={data, pc};
    Object o;
    int n=0;
    ...
    if(n==0 && data.size()>0)
      throw new ParadeException("No manager supports operation "+operationName);
  }
  ...
  static Iterator list(HashMap map,  Producer p, String s)
  {
      ...
      if(o==null)
	{
	  ...
	  if(config.getProperty(s)!=null)
	    {
	      StringTokenizer st= new StringTokenizer(config.getProperty(s), ",");
  ...
  public static Object getManager(String classname)
  {
	...
	if(manager==null)
	  {
	    try{
	      manager=Config.class.getClassLoader().loadClass(cl).newInstance();
	    } catch(Throwable e){ throw new RuntimeException("could not load class "+cl+" "+e.getMessage()); }
  ...
  public static Method getMethod(Object o, String name) throws ParadeException
  {
	...
	if(method==null)
	  {
	    ...
	    } catch(Exception e){ method="nothing";
  ...
  {
    if(byteSize<0l) return ("invalid");
    if(byteSize<1l) return ("empty");
    ...
    if(value.length()>3 && value.indexOf('.')>0) // sut decimals on large numbers
  ...
  public static String readableTime(long milis)
  {
    ...
    if(secs<2l) return ("1 second");
    if(secs==2l) return ("2 seconds");
  ...
  public static PrintStream getPrintStreamCVS(JspWriter out)
  {
	...
	public void println(String s)
	  { try{
               ...
               if(s.startsWith("M ")) style="color:blue"; //modified
               if(s.startsWith("A ")) style="color:purple"; //added
               if(s.startsWith("R ")) style="color:purple"; //removed
               if(s.startsWith("U ")) style="color:green"; //updated
               if(s.startsWith("P ")) style="color:green"; //patched
               if(s.startsWith("C ")) style="color:red; font:bold"; //conflict
               if(s.startsWith("? ")) style="color:purple"; //unknown
               if(s.startsWith("< ")) style="background:#ffdddd"; //content removed
               if(s.startsWith("> ")) style="background:lightblue"; //content added
               if(s.startsWith("exec: ")) style="color:black";
               if(s.startsWith("cvs server: ")) style=style;
               if(s.endsWith("was lost")) style="color: brown; background:pink";
               if(s.endsWith(" -- ignored")) style="color: green";
  ...
  public static int exec(Vector v, File dir, PrintStream ps)
  {
    ...
    }catch(IOException e){ ps.println(e); return -1; }
    
    final Process p=p1;
    final PrintStream ps1=ps;
    new Thread(new Runnable(){
      public void run(){    flushTo(new BufferedReader(new InputStreamReader(p.getErrorStream()), 81960),  ps1);}}).start();
  ...
  public static void addCommandOptions(Vector v, String prefix, javax.servlet.jsp.PageContext pc)
  {
    for(Enumeration e= pc.getRequest().getParameterNames(); e.hasMoreElements(); )
      {
  ...
  public static synchronized void reloadLoggingConfig()
  {
	...
	try{
	  ...
	  for(Enumeration e=p.keys(); e.hasMoreElements(); )
	    {
	      String s=(String)e.nextElement();
	      if(s.startsWith("org.makumba.parade-context")&&s.endsWith(".level") )
		{
		  java.util.logging.Logger.getLogger(s.substring(0,s.length()-6));
		}
	    }
  ...
  */
  ...
  class Producer
  {
    ...
    Object produce(String s) { return s.trim(); }
  }
