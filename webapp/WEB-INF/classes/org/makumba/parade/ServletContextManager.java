...
public class ServletContextManager {
    static String reloadLog = Config.paradeBase + "tomcat" + File.separator
            + "logs" + File.separator + "paradeReloadResult.txt";
    ...
    synchronized ServletContainer getServletContainer(
        ...
        if (container == null)
            try {
                container = (ServletContainer) Config.class
                        .getClassLoader()
                        .loadClass(
                                config
                                        .getProperty("parade.servletContext.servletContainer"))
                        .newInstance();
                if (pc != null) {
                    config.put("parade.servletContext.paradeContext",
                            ((javax.servlet.http.HttpServletRequest) pc
                                    .getRequest()).getContextPath());
                    container.makeConfig(config, pc);
                    config.save(new FileOutputStream(fileName),
                            "Parade servlet context config");
                }
    ...
    public void setParadeSimple(java.util.Map row,
        ...
        if (!s.equals("NO WEBINF")) {
            ...
            row.put("servletContext.status", new Integer(
                    getServletContainer(pc).getContextStatus(
                            (String) row.get("servletContext.name"))));
        }
    }

    public void servletContextStartSimple(java.util.Map row,
            javax.servlet.jsp.PageContext pc) {
        setParadeSimple(row, pc);
        if (!isParadeCheck(row))
            row.put("result", getServletContainer(pc).startContext(
                    (String) row.get("servletContext.name")));
    }

    public void servletContextStopSimple(java.util.Map row,
            javax.servlet.jsp.PageContext pc) {
        setParadeSimple(row, pc);
        if (!isParadeCheck(row))
            row.put("result", getServletContainer(pc).stopContext(
                    (String) row.get("servletContext.name")));
    }

    public void servletContextReloadSimple(java.util.Map row,
            javax.servlet.jsp.PageContext pc) {
        // must check if it's not this one
        setParadeSimple(row, pc);
        if (!isParade(row))
            row.put("result", getServletContainer(pc).reloadContext(
                    (String) row.get("servletContext.name")));
        else {
            try {
                ...
                Runtime.getRuntime().exec(
                        antCommand + " -buildfile " + Config.paradeBase
                                + "build.xml reload");
                // final InputStream i=p.getInputStream();
                /*
                 * new Thread(new Runnable(){ public void run(){ try{ int c;
                 * while((c=i.read())!=-1) System.out.print((char)c);
                ...
                loadConfig();
                row.put("reload", new Integer(Integer.parseInt(config
                        .getProperty("parade.servletContext.selfReloadWait"))));

            } catch (IOException e) {
                row.put("result", "Cannot reload Parade " + e);
                e.printStackTrace();
            }
        }
    }

    public void servletContextInstallSimple(java.util.Map row,
            javax.servlet.jsp.PageContext pc) {
        setParadeSimple(row, pc);
        if (!isParadeCheck(row))
    ...
    public void servletContextRemoveSimple(java.util.Map row,
            javax.servlet.jsp.PageContext pc) {
        setParadeSimple(row, pc);
        if (!isParadeCheck(row))
            row.put("result", getServletContainer(pc).unInstallContext(
                    (String) row.get("servletContext.name")));
    }
    ...
    public static String searchWebinf(String original, File p) {
        ...
        for (int i = 0; i < s.length; i++) {
            ...
            if (f.isDirectory()
                    ...
                    && f.toString().indexOf("tomcat" + File.separator + "logs") == -1) {
    ...
    public static boolean isParade(java.util.Map row) {
        try {
            return row.get("parade.path").equals(
                    new File(Config.paradeBase).getCanonicalPath());
        } catch (IOException e) {
            throw new RuntimeException(e.getMessage());
        }
    }
