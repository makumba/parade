...
/** a multiple editor, allowing choiche of file to edit, and basic operations: save, revert, goto line in the current file */
public class MultipleEditor extends Panel 
{
  ...
  public void setStatusDisplay(StatusDisplay sd){ status=sd; }
  ...
  Observer sourceObserver= new Observer()
  {
    public void update(Observable subject, Object event)
	  ...
	  {
	    // we update the current list element (number 0) because probably
	...
	if(((SourcePointer)subject).getChanged())
	  ...
	  {
	    // this only has effect the first time. first event mens "i'm loaded now"
	    revert.enable();
	    save.disable();
	  }
      }
  ...
  public void loadFile(SourcePointer sp, SourcePosition pos)
  {
	...
	// for some reason, bufferChoice.select(choice) does not produce ItemEvent here 
	...
	((Component)e).enable();
      ...
      // if the file is not new in this multiple editor, we change the bufferChoice position
	{
    ...
    // we show status in a separate thread, so the default statuses shown by the browser 
    // will be overwritten
  ...
  /**change the bufferChoice order, put the item selected in bufferChoice first   */
  public void select()
  {
    ...
    lineEditor.addActionListener(new ActionListener()
				 {
				   public void actionPerformed(ActionEvent e)
					 ...
					 ln=Integer.parseInt(s.trim());
				       ...
				       if(ln>0)
					 {
					   getCurrentEditor().pointTo(new SourcePosition(getCurrentEditor().getText(), ln, 0, 0, 0));
					   getCurrentComponent().requestFocus();
					 }
				       else
					 {
					   int index= getCurrentEditor().getText().indexOf(s, getCurrentEditor().getIntPosition());
					   if(index==-1)
					     index=getCurrentEditor().getText().indexOf(s);
					   if(index==-1)
					     index=getCurrentEditor().getText().toLowerCase().indexOf(s.toLowerCase(), getCurrentEditor().getIntPosition());
					   if(index==-1)
					     index=getCurrentEditor().getText().toLowerCase().indexOf(s.toLowerCase());

					   if(index!=-1)
					     {
					       getCurrentEditor().pointTo(new SourcePosition(index, index+s.length()));
					       getCurrentComponent().requestFocus();
					     }
					   else
					     {
					       status.showStatus("Not found: "+s);
					       setLine(((SourcePosition)positions.get(choiceOrder.elementAt(0))));
					     }
					 }
				     }
				 });
